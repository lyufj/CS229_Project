	...
	with tf.Graph().as_default():
		...
		with sess.as_default():
			...
			def predict_step(x_batch):
				...
				#states = sess.run([cnn_rnn.states], feed_dict)
				return predictions

			def states_step(x_batch):
				feed_dict = {
					cnn_rnn.input_x: x_batch,
					cnn_rnn.dropout_keep_prob: 1.0,
					cnn_rnn.batch_size: len(x_batch),
			...
			predict_states = []
			#predictions, predict_labels, states = [], [], []
			i = 0
			for x_batch in batches:
				i = i+1
				print('Currently on ' + str(i))
				#batch_predictions = predict_step(x_batch)[0]
				#batch_pred_test = predict_step(x_batch)
				batch_states = states_step(x_batch)[0][0]
				#batch_states_test = states_step(x_batch)

				#batch_states_test_2 = states_step(x_batch)[0][0]

				#print('shape of batch_predictions is ' + str(batch_predictions.shape))
				#print('batch pred is ' + str(batch_predictions))
				#print('batch_pred_test is ' + str(batch_pred_test))
				#print('shape of batch_states is ' + str(len(batch_states)) + ' and shape of test is ' + str(len(batch_states_test)))
				#print('batch_states is ' + str(batch_states))
				#print('batch_states_test is ' + str(batch_states_test))

				#print('batch_states_test_2 is ' + str(batch_states_test_2))
				#for batch_prediction in batch_predictions:
				#	predictions.append(batch_prediction)
				#	predict_labels.append(labels[batch_prediction])
					#states.append(
				for batch_state in batch_states:
					predict_states.append(batch_state)
			#df['PREDICTED'] = predict_labels
			df['STATES'] = predict_states
