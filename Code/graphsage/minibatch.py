...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
            ...
            batch_size=100, max_degree=25, sampler='uniform',
            **kwargs):
        ...
        if sampler == 'lineardegree':
            self.adj, self.deg = self.construct_adj_high_degree()
            self.test_adj, self.test_deg = self.construct_test_adj_high_degree()
        elif sampler == 'uniform':
            self.adj, self.deg = self.construct_adj()
            self.test_adj, self.test_deg = self.construct_test_adj()
        else:
            raise ValueError('Sampler should be uniform or lineardegree.')
    ...
    def construct_adj_high_degree(self):
        adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree))
        deg = np.zeros((len(self.id2idx),))
        ...
        for nodeid in self.G.nodes():
            ...
            if len(neighbors) == 0:
                continue
            idx = np.argsort(deg[neighbors])#[::-1]
    ...
    def construct_test_adj_high_degree(self):
        ''' Added deg as construct_adj '''
        adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree))
        deg = np.zeros((len(self.id2idx),))
