...
from graphsage.neigh_samplers import UniformNeighborSampler, LinearDegreeNeighborSampler
...
flags.DEFINE_string("sampler", "uniform", "Can be uniform or lineardegree")
...
SAMPLER_FUNCS = {
    ...
    'lineardegree': LinearDegreeNeighborSampler
}
...
def train(train_data, test_data=None):
    ...
    #deg_info = tf.Variable(tf.constant(minibatch.deg, dtype=tf.int32), trainable=False, name="deg_info")
    #np.save('adj.npy', minibatch.adj)
    #np.save('deg.npy', minibatch.deg)

    if FLAGS.model == 'graphsage_mean':
        # Create model
        sampler = SAMPLER_FUNCS[FLAGS.sampler](adj_info)
    ...
    #train_deg_info = tf.assign(deg_info, minibatch.deg)
    val_adj_info = tf.assign(adj_info, minibatch.test_adj)
    #val_deg_info = tf.assign(deg_info, minibatch.test_deg)
